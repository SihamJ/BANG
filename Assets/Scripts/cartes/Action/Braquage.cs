...
public class Braquage : Carte
{
    ...
    public Braquage(int nombre, string figure, int sprite)
    {
        this.nombre = nombre;
        this.figure = figure;
        this.multi = true;
    ...
    public void action(int j1, int j2, ref List<Carte> defausse, ref List<Carte> pioche, ref List<GameObject> players, ref Text scene, ref Text historique)
    {
        ...
        scene.text = players[j1].GetComponent<Joueur>().getPseudo() + " braque " + players[j2].GetComponent<Joueur>().getPseudo() + " ! Haut les mains !";
